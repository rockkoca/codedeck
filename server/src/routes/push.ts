...
pushRoutes.post('/register', async (c) => {
  ...
  // Store token in DB (push_tokens table)
  try {
...
export async function dispatchPush(payload: PushPayload, db: PgDatabase, fcmKey?: string): Promise<void>;
export async function dispatchPush(payload: PushPayload, envOrDb: Env | PgDatabase, fcmKey?: string): Promise<void> {
  let db: PgDatabase;
  let resolvedFcmKey: string | undefined;

  if ('DB' in envOrDb) {
    // Called with Env
    db = (envOrDb as Env).DB;
    resolvedFcmKey = (envOrDb as unknown as Record<string, string>).FCM_SERVER_KEY;
  } else {
    // Called with PgDatabase directly
    db = envOrDb as PgDatabase;
    resolvedFcmKey = fcmKey;
  }
  ...
  for (const { token, platform } of tokens) {
    try {
      if (platform === 'android') {
        await sendFcm(token, payload, resolvedFcmKey);
      } else if (platform === 'ios') {
        // APNs requires a separate key — simplified implementation
        await sendFcm(token, payload, resolvedFcmKey); // FCM can also handle iOS with unified token
      }
    } catch (err) {
