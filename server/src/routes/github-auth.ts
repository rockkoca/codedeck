...
githubAuthRoutes.get('/', async (c): Promise<Response> => {
  const state = signJwt({ nonce: randomHex(16) }, c.env.JWT_SIGNING_KEY, 600);
...
githubAuthRoutes.get('/callback', async (c): Promise<Response> => {
  ...
  const sessionToken = signJwt({ sub: user.id, type: 'web' }, c.env.JWT_SIGNING_KEY, 86400);

  const redirectUrl = new URL(c.env.SERVER_URL);
