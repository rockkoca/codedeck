...
export async function recordIdempotency(
  ...
  await db
    .prepare(
      'INSERT OR IGNORE INTO idempotency_records (key, user_id, response_status, response_body, created_at) VALUES (?, ?, ?, ?, ?)',
    )
 ...
 */
export async function cleanupIdempotencyRecords(db: Env['DB']): Promise<number> {
